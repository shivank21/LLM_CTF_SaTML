...
def greedy_ec(prompt, model, tokenizer, num_iters=20):
    ...
    such that the softmax score for the adversarial class is maximized. It repeats
    this process until the adv classifier labels the prompt as adversarial. If the
    model does not label the prompt as adversarial, the algorithm declares
    ...
    Returns:
        True if the prompt is adversarial, False otherwise
    '''
    ...
    # If the prompt is initially labelled adversarial, return True
    # Class 1: Safe, Class 0: Adv
    if output_class == 0:
        return True
    
    # Otherwise, erase tokens until the prompt is labelled adversarial
    for _ in range(num_iters):
        ...
        # Find index that maximizes the adversarial class softmax score
