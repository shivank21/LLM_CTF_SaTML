...
from defence import is_asking
from defence import progress_bar, erase_and_check, erase_and_check_smoothing
...
parser.add_argument('--eval_type', type=str, default="safe", choices=["safe", "adv", "smoothing", "empirical", "grad_ec", "greedy_ec"],
...
parser.add_argument('--safe_prompts', type=str, default="data/safe_prompts.txt",help='Path to normal prompts')
parser.add_argument('--harmful_prompts', type=str, default="data/harmful_prompts.txt",help='Path to adversarial prompts(Asking secret)')
...
adv_prompts_file = args.adv_prompts
...
if eval_type == "safe":
    ...
    # Check if the prompts are adv
    count_safe = 0
    start_time = time.time()
    for i in range(num_prompts):
        prompt = prompts[i]
        if args.append_adv: prompt += adv_suffix
        adv = erase_and_check(prompt, pipeline, tokenizer, max_erase=max_erase, num_adv=num_adv, randomized=randomize,
    ...
    for adv_tok in range(0, 21, 2):
        ...
        for i in range(num_prompts):
            ...
            elapsed_time = current_time - start_time
            time_per_prompt = elapsed_time / (i + 1)
            percent_adv = count_harmful / (i + 1) * 100
            print("    Checking safety... " + progress_bar((i + 1) / num_prompts) \
                + f' Detected harmful = {percent_adv:5.1f}%' \
                + f' Time/prompt = {time_per_prompt:5.1f}s', end="\r", flush=True)
            
        print("")
            
        # Save results
        emp_results[str(dict(adv_tok = adv_tok))] = dict(percent_harmful = percent_adv, time_per_prompt = time_per_prompt)
    ...
    for adv_tok in range(0, 21, 2):
        ...
        for i in range(num_prompts):
            prompt = prompts[i]
            adv, _ = grad_ec(prompt, model, tokenizer, model.distilbert.embeddings.word_embeddings,
    ...
    print("Evaluating smoothing-based certificates on harmful prompts from: " + adv_prompts_file)
    # Load prompts from text file
    with open(adv_prompts_file, "r") as f:
        prompts = f.readlines()
        prompts = [prompt.strip() for prompt in prompts]
    ...
    # Harmful prompts
    print("\nEvaluating adv prompts from: " + adv_prompts_file + "\n")
    # Load prompts from text file
    with open(adv_prompts_file, "r") as f:
        prompts = f.readlines()
        prompts = [prompt.strip() for prompt in prompts]
